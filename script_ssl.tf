...
#      --account-name ${azurerm_storage_account.ssl_cert.name} \
#      -c ${azurerm_storage_container.ssl_cert.name} \
...
#      --account-name ${azurerm_storage_account.ssl.name} \
#      -c ${azurerm_storage_container.ssl.name} \
#      -n .well-known/acme-challenge/$${CERTBOT_TOKEN} \
#      --only-show-errors 2>&1 > /dev/null
#   EOF
# }
...
# locals {
#   ssl_contact_email      = "jpaillusseau.ext@simplon.co"
...
#           -d ${azurerm_public_ip.app.fqdn} > /dev/null

#   openssl pkcs12 -export \
#           -out ${local.ssl_cert_pfx_path} \
#           -inkey ${local.ssl_certbot_config_dir}/live/${azurerm_public_ip.app.fqdn}/privkey.pem \
#           -in ${local.ssl_certbot_config_dir}/live/${azurerm_public_ip.app.fqdn}/fullchain.pem \
#           -passout pass:"$${SSL_PASSWD}" > /dev/null

#   az keyvault certificate import --vault-name ${azurerm_key_vault.ssl.name} \
...
#   depends_on = [
#     azurerm_key_vault_access_policy.default_ssl,
#     #azurerm_application_gateway.app
#   ]
#   triggers = {
#     fqdn = azurerm_public_ip.app.fqdn
#   }
